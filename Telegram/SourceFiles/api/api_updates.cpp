...
void Updates::addActiveChat(rpl::producer<PeerData*> chat) {
	...
	std::move(
		...
		auto &entry = _activeChats[key];
		const auto was = entry.peer;
		if (was == peer) {
			return;
		}
		const auto already = peer && isActiveChat(peer);
		entry.peer = peer;
		const auto still = was && isActiveChat(was);
		if (was && !still) {
