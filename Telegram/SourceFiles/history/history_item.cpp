...
void HistoryItem::FillForwardedInfo(
	...
	if (const auto savedFromMsgId = data.vfrom_message_id().v) {
		config.savedFromPeer = peerFromTdbChat(data.vfrom_chat_id());
		config.savedFromMsgId = savedFromMsgId;
	}
}
...
std::unique_ptr<Data::Media> HistoryItem::CreateMedia(
	...
	return content.match([&](const TLDmessageText &data) -> Result {
		...
		return data.vweb_page()
			? std::make_unique<Data::MediaWebPage>(
				item,
				owner.processWebpage(*data.vweb_page()),
				flags)
			: nullptr;
		...
		return std::make_unique<Data::MediaGiveaway>(
			item,
			Data::ComputeGiveawayData(item, data),
			(data.vsticker()
...
void HistoryItem::updateInteractionInfo(
	...
	if (fields) {
		...
		updateReactions(fields->vreactions().v);
	} else {
		setForwardsCount(0);
		clearReplies();
	}
